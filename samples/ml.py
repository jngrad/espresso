...
print("Tune p3m")
p3m = espressomd.electrostatics.P3M(prefactor=coulomb_prefactor, accuracy=1e-1, mesh=[14,14,14], cao=5)
...
p_label = "Solvent" # "Colloid"
n_steps = 100

p_slice = system.part.select(type=types[p_label])
id_range = list(p_slice.id)
obs_pos = espressomd.observables.ParticlePositions(ids=id_range)
ml_pos = lambda : np.array(system.analysis.call_method("particle_properties", pids=id_range, properties=["pos"])["pos"])
obs_data = obs_pos.calculate()
ml_data = ml_pos()
np.testing.assert_allclose(ml_data, obs_data, atol=1e-10)


print(f"benchmarking observables for {len(id_range)} particles on {system.cell_system.get_state()['n_nodes']} MPI ranks in {n_steps} steps")
...
tock = time.time()
print(f"{10 * (tock - tick):.1f}s for particle slice")
...
tock = time.time()
print(f"{tock - tick:.1f}s for observable")
...
tock = time.time()
print(f"{tock - tick:.1f}s for machine-learning")
