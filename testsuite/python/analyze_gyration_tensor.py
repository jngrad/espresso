...
class AnalyzeGyration(ut.TestCase):
    ...
    def test_gyration_tensor_cube(self):
        ...
        rg = self.system.analysis.calc_rg(
            chain_start=0,
            number_of_chains=1,
            chain_length=self.cube_len**3)[0]
    ...
    def test_mom_intertia(self):
        ...
        sqr_dist = np.sum(
            (self.system.analysis.center_of_mass(
                p_type=0) -
                self.system.part.select(
                type=0).pos)**2,
            axis=0)
        mom_I = self.system.analysis.moment_of_inertia_matrix(p_type=0)
        # the cube case should have zero as off- diagonal components
        self.assertTrue(np.allclose([mom_I[0, 1], mom_I[0, 2], mom_I[1, 2],
                                     mom_I[1, 0], mom_I[2, 0], mom_I[2, 1]], np.zeros(6), atol=1e-6))
