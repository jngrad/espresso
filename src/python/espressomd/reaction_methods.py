...
import os
if os.environ.get("LEGACY_MONTE_CARLO", "0") == "0":
    ...
    class MonteCarloMethod:
        ...
        def __init__(self, **kwargs):
            ...
            assert self.system.cell_system.get_state()["n_nodes"] == 1, "no MPI"
            ...
            self.system.call_method("setup_reaction_algo", seed=kwargs["seed"])
        ...
        def displacement_mc_move(self, ptype, n_particles):
            ...
            for i in range(n_particles):
                ...
                while p_id in drawn_pids:
                    ...
                    p_id = self.system.call_method("get_random_p_id_of_type", ptype=ptype)
                ...
                new_pos = self.system.call_method("get_random_position_in_box")
                new_vel = self.system.call_method("get_random_velocity_vector") * math.sqrt(self.kT / p.mass)
                print(f"change pid={p_id} v={p.v}->{new_vel}")
                sys.stdout.flush()
        ...
        def displacement_mc_move_for_particles_of_type(
            ...
            pp = self.system.call_method("reaction_make_displacement_mc_move_attempt_uniform")
            print(f"metropolis: uniform={pp:.14f} < {bf:.14f}")
            if pp < bf:
                # accept
    ...
    class ReactionAlgorithm(MonteCarloMethod):
        ...
        def make_reaction_attempt(self, reaction):
            ...
            for index in range(minimum_number_of_types):
                ...
                for _ in range(size):
                    random_pid = self.system.call_method("get_random_p_id_of_type", ptype=r_type)
                    p = self.system.part.by_id(random_pid)
                    print(f'change pid={random_pid} {p.type}->{p_type}')
                    sys.stdout.flush()
        ...
        def delete_created_particles(self):
            for particle_info in self.particle_changes["created"]:
                self.system.call_method("reaction_remove_particle", pid=particle_info["pid"])
                self.system.part.by_id(particle_info["pid"]).remove()
                self.free_particle_id(particle_info["pid"])
                print(f'remove pid={particle_info["pid"]}')
                sys.stdout.flush()
        ...
        def restore_system(self):
            ...
            for particle_info in self.particle_changes["changed"] + \
                ...
                print(f'change pid={p.id} change to->{particle_info}')
                sys.stdout.flush()
        ...
        def hide_particle(self, pid):
            ...
            print(f'change pid={pid} {p.type}->{self.non_interacting_type}')
            sys.stdout.flush()
        ...
        def create_particle(self, ptype):
            ...
            pos, vel = self.system.call_method("reaction_add_particle", pid=pid, new_type=ptype)
            print(f'create pid={pid} NA->{ptype} p={pos} v={vel * math.sqrt(self.kT)}')
            sys.stdout.flush()
            self.system.part.add(id=pid, type=ptype, q=self.default_charges[ptype],
                                 pos=pos, v=vel * math.sqrt(self.kT))
            return pid
        ...
        def reaction(self, steps=1):
            ...
            """
            self.system.call_method("rebuild_type_map")
            self.setup_bookkeeping_of_empty_pids()
            E_pot = self.system.analysis.potential_energy()
            n_reactions = len(self.reactions)
            for _ in range(steps):
                i = self.system.call_method("get_random_reaction_index", size=n_reactions)
        ...
        def generic_oneway_reaction(self, reaction, E_pot_old):
            ...
            try:
                ...
                self.particle_inside_exclusion_range_touched = False
                print(f"all_reactant_particles_exist={int(self.all_reactant_particles_exist(reaction))}")
                sys.stdout.flush()
                ...
                if self.particle_inside_exclusion_range_touched:
                    # reject trial move
                    print(f"reject due to exclusion radius")
                    sys.stdout.flush()
                    self.restore_system()
                    self.particle_inside_exclusion_range_touched = False
                    return E_pot_old
                ...
                pp = self.system.call_method("reaction_make_displacement_mc_move_attempt")
                print(f"metropolis: {pp:.14f} >= {ln_bf:.14f}")
                sys.stdout.flush()
                if pp >= ln_bf:
                    # reject trial move
                    self.restore_system()
    ...
    class ReactionAlgorithm(ScriptInterfaceHelper):
        ...
        def __init__(self, **kwargs):
            if "system" in kwargs:
                assert kwargs["system"].cell_system.get_state()["n_nodes"] == 1, "no MPI"
                print("setup_reaction_algo")
                kwargs["system"].call_method("setup_reaction_algo", seed=kwargs["seed"])
                del kwargs["system"]
