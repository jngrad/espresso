...
#include "ReactionKernelBulk_2_double_precision_CUDA.h"
#include "ReactionKernelBulk_2_single_precision_CUDA.h"

#include "ReactionKernelBulk_3_double_precision_CUDA.h"
#include "ReactionKernelBulk_3_single_precision_CUDA.h"

#include "ReactionKernelBulk_4_double_precision_CUDA.h"
#include "ReactionKernelBulk_4_single_precision_CUDA.h"

#include "ReactionKernelBulk_5_double_precision_CUDA.h"
...
struct KernelTraitGPU {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_1_double_precision_CUDA;
};
...
template <> struct KernelTraitGPU<double, 3> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_3_double_precision_CUDA;
};
...
template <> struct KernelTraitGPU<float, 1> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_1_single_precision_CUDA;
};

template <> struct KernelTraitGPU<float, 2> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_2_single_precision_CUDA;
};

template <> struct KernelTraitGPU<float, 3> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_3_single_precision_CUDA;
};

template <> struct KernelTraitGPU<float, 4> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_4_single_precision_CUDA;
};

template <> struct KernelTraitGPU<float, 5> {
  using ReactionKernelBulkGPU =
      pystencils::ReactionKernelBulk_5_single_precision_CUDA;
};

template <typename FloatType, class Reactant, std::size_t... ints>
auto get_kernel_impl_gpu(
    const std::vector<std::shared_ptr<Reactant>> &reactants,
    const double coefficient, std::index_sequence<ints...> int_seq) {
  auto kernel = std::make_shared<typename KernelTraitGPU<
      FloatType, int_seq.size()>::ReactionKernelBulkGPU>(
      walberla::BlockDataID(
          reactants[ints]->get_species()->get_density_id())...,
      numeric_cast<FloatType>(reactants[ints]->get_order())...,
      numeric_cast<FloatType>(coefficient),
