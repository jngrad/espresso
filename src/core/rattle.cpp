...
static void init_correction_vector(const ParticleRange &particles,
                                   const ParticleRange &ghost_particles) {
  auto reset_force = [](Particle &p) { p.rattle.correction = {0, 0, 0}; };
 ...
 * @param particles particle range
 */
static void apply_position_correction(const ParticleRange &particles) {
 ...
 * @param kernel function to call for correction calculation
 * @return true if correction is necessary
 */
template <typename Kernel>
static bool compute_corr_vec(CellStructure &cs, Kernel kernel) {
 ...
 * @return true if a correction is necessary
 */
...
/** Iterate the positional shake algorithm
 *
 * @param cs cell structure
 */
void correct_pos_shake(CellStructure &cs) {
  ...
  for (cnt = 0; cnt < SHAKE_MAX_ITERATIONS; ++cnt) {
    ...
    bool const repeat_ = compute_corr_vec(cs, add_pos_corr_vec);
    bool const repeat =
    ...
    apply_position_correction(particles);
...
/** Apply the velocity correction to the particles.
 *
 * @param particles particle range
 */
...
void correct_vel_shake(CellStructure &cs) {
  ...
  for (cnt = 0; cnt < SHAKE_MAX_ITERATIONS; ++cnt) {
    init_correction_vector(particles, ghost_particles);
    bool const repeat_ = compute_corr_vec(cs, add_vel_corr_vec);
    bool const repeat =
        boost::mpi::all_reduce(comm_cart, repeat_, std::logical_or<bool>());
